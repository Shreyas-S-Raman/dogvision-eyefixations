...
Instructions:
    ...
    To run this script, call python3 video_frame_extraction.py --dog_name {name} on the terminal (ensure to have the libraries below installed in a virtual environment)
'''
import pandas as pd
...
BASE_PATH = os.path.relpath('./Dog_Video_Frames')
...
def extract_fixation_frames(video_path=None, save_base_path=None):
    ...
    if not os.path.isdir(os.path.join(BASE_PATH, dog_name,'frames')):
        os.mkdir(os.path.join(BASE_PATH, dog_name, 'frames'))

    SAVE_DIR = os.path.join(BASE_PATH, dog_name, 'frames')
    ...
    while True:
        ...
        if ret:
            ...
            if curr_frame==int(target_frame):
                ...
                while fixation_frames[target_idx] < target_frame + frame_rate:
                    target_idx +=1
